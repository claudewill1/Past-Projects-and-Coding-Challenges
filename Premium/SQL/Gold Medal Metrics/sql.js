...
const createGoldMedalTable = () => {
  return `CREATE TABLE GoldMedal (id INTEGER NOT NULL PRIMARY KEY,
...
const mostMedaledAthlete = country => {
  return `SELECT name FROM GoldMedal WHERE country = '${country}' GROUP BY name ORDER BY
  COUNT(*) DESC LIMIT 1;`;
};
...
const orderedSports = (country, field, isAscending) => {
  ...
  return `SELECT sport, COUNT(sport) AS count, (COUNT(sport) * 100 / (select COUNT(*) FROM GoldMedal WHERE country = '${country}')) 
  AS percent FROM GoldMedal WHERE country = '${country}' GROUP BY sport ${orderingString};`;
};
